...
async function createStores() {
  for (let state of states) {
    for (let i = 0; i < 2; i++) {
      ...
      const latitude = faker.location.latitude({min:-33, max: 5}).toString();
      const longitude = faker.location.longitude({min:-70, max: 30}).toString();
      const telephoneNumber = faker.phone.number();
      const emailAddress = faker.internet.email();
      const content = faker.lorem.sentence();

      // Criando a loja no banco de dados
